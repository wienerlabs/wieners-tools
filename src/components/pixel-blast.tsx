...
export default function PixelBlast({
  ...
  useEffect(() => {
    ...
    if (!threeRef.current) mustReinit = true;
      ...
      const keys: (keyof typeof cfg)[] = ["antialias", "liquid", "noiseAmount"];
      for (const k of keys) {
    ...
    if (mustReinit) {
      ...
      const onPointerDown = (e: PointerEvent) => {
        ...
        const click = (uniforms.uClickPos.value as THREE.Vector2[])[ix];
        click.set(fx, fy);
      ...
      const animate = () => {
        ...
        uniforms.uTime.value = (timeOffset + clock.getElapsedTime() * speedRef.current) as number;
        if (liquidEffect) {
        ...
        if (composer) {
          ...
          composer.passes.forEach((p) => {
            const eff = (p as { effects?: Effect[] }).effects;
            if (eff) {
              eff.forEach((eff2) => {
                const u = eff2.uniforms?.get("uTime");
                if (u) u.value = uniforms.uTime.value;
              });
            }
          });
  ...
  return (
    <div
      ref={containerRef}
      className={`ws-pixel-blast ${className ?? ""}`.trim()}
      style={style}
